...
param (
    ...
    [Parameter(Mandatory = $false)]
    [string] $TemplateUri = 'https://raw.githubusercontent.com/jsmallwood/AzureOptimizationEngine/master/azuredeploy.json',
...
function CreateSelfSignedCertificate([string] $certificateName, [string] $selfSignedCertPlainPassword,[string] $certPath, [string] $certPathCer, [int] $selfSignedCertNoOfMonthsUntilExpired )
{
...
try {
    Invoke-RestMethod -Method Get -Uri $TemplateUri | Out-Null
...
if ("Y", "y" -contains $continueInput) {
    ...
    {
        #region Upgrade Azure Storage Account Containers
        ...
        #region Upgrade Azure Automation Runbooks
        ...
        #region Import Modules into Azure Automation Account
        ...
        #region Upgrade Azure Automation Schedules
    ...
    #region Azure Automation Run As Account
    ...
    do {
        ...
        try {
            ...
            #region Additional Stored Procedures

            $Conn = New-Object System.Data.SqlClient.SqlConnection("Server=tcp:$sqlServerEndpoint,1433;Database=$databaseName;User ID=$sqlAdmin;Password=$sqlPassPlain;Trusted_Connection=False;Encrypt=True;Connection Timeout=$SqlTimeout;")
            $Conn.Open()

            $createTableQuery = Get-Content -Path "./model/recommendations-category-sp.sql"
            $Cmd = new-object system.Data.SqlClient.SqlCommand
            $Cmd.Connection = $Conn
            $Cmd.CommandTimeout = $SqlTimeout
            $Cmd.CommandText = $createTableQuery
            ...
            $createTableQuery = Get-Content -Path "./model/recommendations-category-impact-sp.sql"
            $Cmd = new-object system.Data.SqlClient.SqlCommand
            $Cmd.Connection = $Conn
            $Cmd.CommandTimeout = $SqlTimeout
            $Cmd.CommandText = $createTableQuery
            ...
            $createTableQuery = Get-Content -Path "./model/recommendations-security-sp.sql"
            $Cmd = new-object system.Data.SqlClient.SqlCommand
            $Cmd.Connection = $Conn
            $Cmd.CommandTimeout = $SqlTimeout
            $Cmd.CommandText = $createTableQuery
            ...
            $createTableQuery = Get-Content -Path "./model/recommendations-operations-sp.sql"
            $Cmd = new-object system.Data.SqlClient.SqlCommand
            $Cmd.Connection = $Conn
            $Cmd.CommandTimeout = $SqlTimeout
            $Cmd.CommandText = $createTableQuery
            ...
            $createTableQuery = Get-Content -Path "./model/recommendations-ha-sp.sql"
            $Cmd = new-object system.Data.SqlClient.SqlCommand
            $Cmd.Connection = $Conn
            $Cmd.CommandTimeout = $SqlTimeout
            $Cmd.CommandText = $createTableQuery
    ...
    #region Azure Monitor Workbooks
