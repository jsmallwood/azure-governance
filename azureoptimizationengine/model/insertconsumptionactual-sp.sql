/****** Object:  StoredProcedure [dbo].[usp_InsertConsumptionActual]    Script Date: 4/25/2023 2:28:02 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER PROCEDURE [dbo].[usp_InsertConsumptionActual]
(
...
BEGIN
	...
	SET NOCOUNT ON
	IF NOT EXISTS(SELECT 1 FROM [dbo].[ConsumptionActual] WHERE Date=@Date and ChargeType = @ChargeType and Cost = @Cost and MeterId = @MeterId and MeterName = @MeterName and ProductId = @ProductId and ResourceId = @ResourceId and UnitPrice = @UnitPrice)
		BEGIN
			INSERT INTO [dbo].[ConsumptionActual] 
