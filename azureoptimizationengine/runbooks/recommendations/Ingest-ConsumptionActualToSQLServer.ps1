...
$storageAccountSinkContainer = Get-AutomationVariable -Name  "AzureOptimization_ConsumptionActualContainer" -ErrorAction SilentlyContinue

Write-Output "$($storageAccountSinkContainer)"
if ([string]::IsNullOrEmpty($storageAccountSinkContainer)) {
    $storageAccountSinkContainer = "consumptionactualexports"
}
...
$consumptionTable = "ConsumptionActual"
...
foreach ($blob in $unprocessedBlobs) {
    ...
    for ($i = 0; $i -lt $objCsv.Count; $i++)
    {
        ...
        $sqlStatement = "EXEC dbo.usp_InsertConsumptionActual $($values);"

        Write-Output "Inserting row $($i) of Blob $($Blob.Name) into Consumption Actual Table..."
