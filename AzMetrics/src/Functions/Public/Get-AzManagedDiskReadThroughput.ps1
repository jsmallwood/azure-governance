...
Function Get-AzManagedDiskReadThroughput
{
    [CmdletBinding(
    ...
    process {
        ...
        try {
            $objMetric = Get-AzMetric -ResourceId $ResourceId -MetricName 'Composite Disk Read Bytes/sec' -TimeGrain $TimeGrain -StartTime $StartTime -EndTime $EndTime @boundParameters -ErrorAction $errorAction
        } catch {
            return $_
        }
...
Export-ModuleMember -Function Get-AzManagedDiskReadThroughput