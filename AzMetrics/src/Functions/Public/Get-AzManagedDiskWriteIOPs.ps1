...
Function Get-AzManagedWriteIOPs
{
    [CmdletBinding(
    ...
    process {
        ...
        try {
            $objMetric = Get-AzMetric -ResourceId $ResourceId -MetricName 'Composite Disk Write Operations/sec' -TimeGrain $TimeGrain -StartTime $StartTime -EndTime $EndTime @boundParameters -ErrorAction $errorAction
        } catch {
            return $_
        }
...
Export-ModuleMember -Function Get-AzManagedDiskWriteIOPs