       ...
       for($i=0; $i -le $LookBackPeriodInDays; $i++)
        {
            ...
            $ExportFile = "$($ExportPath)\$($StartDate)_to_$($EndDate)_ReservationTransactions.json"
            ...
            try {
                $uri = "https://management.azure.com/$($Scope)/providers/Microsoft.Consumption/reservationTransactions?`$filter=properties/eventDate+ge+$($StartDate)+AND+properties/eventDate+le+$($EndDate)&api-version=$($apiVersion)"
...
Export-ModuleMember -Function Get-AzRITransactions