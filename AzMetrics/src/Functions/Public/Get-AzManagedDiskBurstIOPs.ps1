...
Function Get-AzManagedDiskBurstIOPs
{
    [CmdletBinding(
    ...
    process {
        ...
        try {
            $objMetric = Get-AzMetric -ResourceId $ResourceId -MetricName 'DiskPaidBurstIOPS' -TimeGrain $TimeGrain -StartTime $StartTime -EndTime $EndTime @boundParameters -ErrorAction $errorAction
        } catch {
            return $_
        }

        if(($objMetric.Data.Count -eq 0) -and ($objMetric.TimeSeries.Count -eq 0))
        {
            $avg = 0
            $avgUsed = 'none'
        }
        ...
        if($avgUsed -eq 'none') { $avgMax = 0; $avgMin = 0; $timeSeriesAvg = 0; $dataAvg = 0 }
...
Export-ModuleMember -Function Get-AzManagedDiskBurstIOPs