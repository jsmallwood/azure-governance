...
Function Get-AzVmNetworkInTotalBytes
{
    [CmdletBinding(
    ...
    process {
        ...
        $AggregationTypes | % {
            $Aggregation = $_
            try {
                $objMetric = Get-AzMetric -ResourceId $ResourceId -MetricName 'Network In Total' -TimeGrain $TimeGrain -StartTime $StartTime -EndTime $EndTime -AggregationType $Aggregation
...
Export-ModuleMember -Function Get-AzVmNetworkInTotalBytes