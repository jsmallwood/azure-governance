       ...
       for($i=0; $i -le $LookBackPeriodInDays; $i++)
        {
            ...
            $ExportFile = "$($ExportPath)\$($StartDate)_to_$($EndDate)_ReservationDetails.json"
            ...
            try {
                $uri = "https://management.azure.com/$($Scope)/providers/Microsoft.Consumption/reservationDetails?startDate=$($StartDate)&endDate=$($EndDate)&`$filter=properties/UsageDate+ge+$($StartDate)+AND+properties/UsageDate+le+$($EndDate)&api-version=$($apiVersion)"
...
Export-ModuleMember -Function Get-AzRIDetails