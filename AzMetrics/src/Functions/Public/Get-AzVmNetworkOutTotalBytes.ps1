...
Function Get-AzVmNetworkOutTotalBytes
{
    [CmdletBinding(
    ...
    begin {
        ...
        #region Initialize Variables
            ...
            [String[]] $AggregationTypes = "Average", "Maximum", "Minimum", "Total"
        #endregion
    ...
    process {
        ...
        $AggregationTypes | % {
            $Aggregation = $_
            try {
                $objMetric = Get-AzMetric -ResourceId $ResourceId -MetricName 'Network Out Total' -TimeGrain $TimeGrain -StartTime $StartTime -EndTime $EndTime -AggregationType $Aggregation
                ...
                {
                    $objMetric.Data | ForEach-Object { $data += $_.Total }
                    $value = ($data | Measure-Object -Sum).Sum
                    $objResult| Add-Member -MemberType NoteProperty -Name Total -Value $value
                }
...
Export-ModuleMember -Function Get-AzVmNetworkOutTotalBytes