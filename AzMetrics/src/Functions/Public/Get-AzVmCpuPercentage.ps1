...
Function Get-AzVmCpuPercentage
{
    [CmdletBinding(
    ...
    process {
        ...
        $AggregationTypes | % {
            $Aggregation = $_
            try {
                $objMetric = Get-AzMetric -ResourceId $ResourceId -MetricName 'Percentage CPU' -TimeGrain $TimeGrain -StartTime $StartTime -EndTime $EndTime -AggregationType $Aggregation
...
Export-ModuleMember -Function Get-AzVmCpuPercentage