...
Function Get-AzManagedDiskReadIOPs
{
    [CmdletBinding(
    ...
    process {
        try {
            $objMetric = Get-AzMetric -ResourceId $ResourceId -MetricName 'Composite Disk Read Operations/sec' -TimeGrain $TimeGrain -StartTime $StartTime -EndTime $EndTime @boundParameters -ErrorAction $errorAction
        } catch {
            return $_
        }
...
Export-ModuleMember -Function Get-AzManagedDiskReadIOPs