...
Function Get-AzManagedDiskIOPs
{
    [CmdletBinding(
    ...
    process {
        ...
        try {
            ...
            $objWriteBytes = Get-AzManagedDiskWriteIOPs -ResourceId $ResourceId -TimeGrain $TimeGrain -StartTime $StartTime -EndTime $EndTime @boundParameters -ErrorAction $errorAction

            # Disk Read Operations/sec
            $objReadBytes = Get-AzManagedDiskReadIOPs -ResourceId $ResourceId -TimeGrain $TimeGrain -StartTime $StartTime -EndTime $EndTime @boundParameters -ErrorAction $errorAction
        } catch {
            return $_
        }
        # Return IOPs
...
Export-ModuleMember -Function Get-AzManagedDiskIOPs