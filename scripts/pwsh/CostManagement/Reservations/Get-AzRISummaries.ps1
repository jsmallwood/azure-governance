...
Get-AzRISummariesAll

This will list Reservation Summaries under the Billing Account for the last 30 days
...
Function Get-AzRISummariesAll
{
    ...
    begin
    {
        ...
        #region Create Scope
            ...
            if(!($scope) -or ($scope -eq 'CurrentUser')) { $scope = New-ScopeBillingAccountId -billingAccountId (Get-AzBillingAccount -ExpandBillingProfile).Name }
            #endregion
...
#$ExportFileName = $settings.ExportFileName.Consumption.Reservations.Summaries.Name


$objSummaries = Get-AzRISummariesAll `
    -billingAccountId $settings.BillingAccountId `
    -grain daily `
    -startDate (Get-Date).AddDays(-7) `
    -endDate (Get-Date)
...
$objSummaries | ConvertTo-Json | Out-File "$ExportPath\2021-11-18 RI_Summaries.json"
...
$ExportFileName = $settings.ExportFileName.Consumption.Reservations.Summaries
